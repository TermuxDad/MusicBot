...
class Thumbnail:
    def __init__(self):
        self.title_font_path = "AnonXMusic/assets/Poppins-ExtraBold.ttf"
        self.subtitle_font_path = "AnonXMusic/assets/Raleway-Bold.ttf"
    ...
    async def generate(
        ...
        bot_name = getattr(app, "name", None) or "Music Bot"
        ...
        if not output_path:
            ...
            output_path = os.path.join("cache", f"thumb_{media_id}.jpg")
...
# Existing MusicBot plugins call: await get_thumb(videoid, user_id)
async def get_thumb(videoid, user_id, title=None, duration=None, thumbnail=None,
                    views=None, channel=None):
    output_path = os.path.join("cache", f"{videoid}_{user_id}.jpg")
    ...
    try:
        if not title or not thumbnail:
            ...
            title = title or result.get("title") or "Unknown Title"
            thumbnail = thumbnail or (
                result.get("thumbnails", [{}])[0].get("url", "").split("?")[0]
            )
            duration = duration or result.get("duration") or "Unknown"
            channel = channel or result.get("channel", {}).get("name") or "Unknown"
            views = views or result.get("viewCount", {}).get("short") or "Unknown Views"
        ...
        thumb = Thumbnail()
        return await thumb.generate(media, output_path=output_path)
