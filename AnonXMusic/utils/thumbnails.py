...
class Thumbnail:
    ...
    async def generate(
        ...
        """Entrypoint used by the bot (auro/core/calls.py):
...
async def get_thumb(
    ...
    """
    MusicBot-compatible entry point.

    Existing MusicBot modules call:
        await get_thumb(videoid, user_id)

    or may provide metadata. The Ruma thumbnail renderer is kept behind
    this function so no call.py/skip.py/callback.py changes are required.
    """
    cache_dir = "cache"
    os.makedirs(cache_dir, exist_ok=True)
    output_path = os.path.join(cache_dir, f"{videoid}_{user_id}.jpg")
    ...
    try:
        if not title or not thumbnail:
            ...
            item = data[0]
            title = item.get("title") or title or "Unknown Title"
            thumbnail = (
                (item.get("thumbnails") or [{}])[0].get("url")
                or thumbnail
            )
            channel = (
                (item.get("channel") or {}).get("name")
                or channel
                or "Unknown"
            )
        ...
        # The renderer accepts any object exposing these attributes.
        ...
        # Keep the bot usable if the fancy renderer fails. Generate a
        # simple fallback from the same YouTube artwork.
        print(f"[Thumbnail] renderer failed for {videoid}: {exc!r}")
        ...
        if thumbnail:
            try:
                ...
                # Validate that PIL can actually open the downloaded image.
                def _validate():
                ...
                print(
                    f"[Thumbnail] fallback failed for {videoid}: "
                    f"{fallback_exc!r}"
                )
